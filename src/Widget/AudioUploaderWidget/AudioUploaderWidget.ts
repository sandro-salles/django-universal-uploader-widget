...
export class AudioUploaderWidget {
  element: HTMLElement;
  ...
  renderWidget() {
    ...
    if (this.file) {
      ...
      this.element.appendChild(
        renderMediaPreview(url, this.canDelete, this.canPreview, 'audio')
      );
    } else if (this.raw) {
      this.element.appendChild(
        renderMediaPreview(this.raw, this.canDelete, this.canPreview, 'audio')
      );
    }
    Array.from(this.element.querySelectorAll(".uuw-file-preview")).forEach(
